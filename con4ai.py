...
class GameView(object):
    ...
    def __init__(self, width=640, height=400, fps=30): # default width and height set to 640x400w
        """Initialize pygame, window, background, font,...
        """
        pygame.init() # initialize pygame
        pygame.display.set_caption("Conn4AI") # Set title of Pygame window
        self.width = width
        self.height = height
        self.screen = pygame.display.set_mode((self.width, self.height), pygame.FULLSCREEN)
        self.background = pygame.Surface(self.screen.get_size()).convert()
        self.clock = pygame.time.Clock() # create object to track time
        self.fps = fps # fps set to default of 30
        self.playtime = 0.0
    ...
    def run(self, game_mode, iterations=1):
        ...
        while (iterations > 0):
            ...
            self.background.fill(BLACK) # black background in game screen
            self.game_board.draw(self.background)
            game_over = False
            turn_ended = False
            uninitialized = True
            current_type = random.randint(1, 2) # choose a random type of coin
            if game_mode == "single":
            ...
            (first_slot_X, first_slot_Y) = self.game_board.get_slot(0, 0).get_position() # get top left corner x/y
            ...
            while not game_over:
                ...
                coin.draw(self.background) # draw coin at slot

                current_player = self.p1 if p1_turn else self.p2 # change player turns
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_ESCAPE:
                            game_over = True
                        if event.key == pygame.K_RIGHT and human_turn: # move coin right
                            if (coin.get_column() + 1 < self.board_cols):
                                coin.move_right(self.background)

                        elif event.key == pygame.K_LEFT and human_turn:     # move coin left
                            if (coin.get_column() - 1 >= 0):
                                coin.move_left(self.background)

                        elif event.key == pygame.K_RETURN and human_turn and not coin_inserted: # drop coin
                            try:
    ...
    def draw_menu(self):
        ...
        self.title_surface = font.render('CONN4AI', True, BLACK)
        fw, fh = font.size('CONN4AI')
        ...
        self.rect1 = self.play_surface.get_rect(topleft=((self.width - fw) // 2, 300)) # set a imaginary rectangle
        self.background.blit(self.play_surface, ((self.width - fw) // 2, 300))  # whose topleft coordinate is at ((self.width - fw) // 2, 300))
        ...
        # self.background.blit(computer_play_surface, ((self.width - fw) // 2, 350)) # blit is done to copy text to top of background
        ...
        self.rect2 = self.quit_surface.get_rect(topleft=((self.width - fw) // 2, 400))
        self.background.blit(self.quit_surface, ((self.width - fw) // 2, 400))
    ...
    def draw_game_over(self, winner):
        ...
        self.title_surface = font.render(game_over_text, True, RED)
...
class Board():
    ...
    def __init__(self, num_rows, num_columns):
        ...
        # list comprehension used to store list of slot objects
...
class Slot:
    ...
    def draw(self, background):
        ...
        """
        # pygame.draw.rect(self.surface, BOARDIMG, (0, 0, self.width, self.height))
        # pygame.draw.rect(self.surface, WHITE, (1, 1, self.width - 2, self.height - 2))
        # self.surface = self.surface.convert()
        background.blit(BOARDIMG, (self.x_pos, self.y_pos))
...
class Coin():
    ...
    def draw(self, background):
        ...
        # pygame.draw(self.surface, self.color, (Slot.SIZE // 2, Slot.SIZE // 2), Coin.RADIUS)
        # self.surface = self.surface.convert()
        if self.color == (255,0,0):
            background.blit(REDTOKENIMG, (self.x_pos, self.y_pos))
        else:
            background.blit(BLACKTOKENIMG, (self.x_pos, self.y_pos))
...
if __name__ == "__main__":
    ...
    args = parser.parse_args() # pass default arguments

    GameView(1200, 768).main_menu(int(args.iterations)) # pass screen size to game view and iterations to main menu
